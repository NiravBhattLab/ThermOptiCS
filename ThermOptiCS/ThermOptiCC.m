function [a,TICs,Dir,modModel,TIC_Rxns] = ThermOptiCC(model,tol,TICs,Dir)
% Identifies whether a reaction in the given input model can carry
% thermodynamically feasible flux
%
% USAGE: 
...
if ~exist('TICs', 'var') || isempty(TICs) || ~exist('Dir', 'var') || isempty(Dir)
    [TICs,Dir,TIC_Rxns] = ThermoOptEnumMILP(model);
end
