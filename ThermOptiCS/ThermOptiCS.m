function [Model,bCoreRxns,TICs,Dir,CSM_TIC,CSM_Dir] = ThermOptiCS(model,core,tol,TICs,Dir)
% Thermodynamically feasible gap-filling.
%
% USAGE: 
...
%     tol:       Tolerance value (What the user define as non-zero value).
%
% OUTPUTS:
...
if ~exist('TICs', 'var') || isempty(TICs) || ~exist('Dir', 'var') || isempty(Dir)
    ...
    [a,TICs,Dir] = ThermOptiCC(model,tol,TICs,Dir);
end
...
while ~isempty(core)
    ...
    IDS = findMinNetwork(modelIrrev,intersect(core,IDScc),P,TICmat,rev2irrev,tol,x0);
...
function IDS = findMinNetwork(model,core,P,TICmat,rev2irrev,tol,x0)
...
% bounds
lb = zeros(n,1);lb(core)=tol;
lb = [lb;zeros(n,1)];
ub = ones(n,1)*1000;ub2=ones(n,1);
% ub2(rBlk)=0; ub(rBlk)=0; % type 2
ub = [ub;ub2];
...
solution = solveCobraMILP(MILPproblem,'timeLimit',60);
x=solution.full;
IDS = find(x(n+1:end)>0.5);
end
...
% bounds
lb = zeros(2*n,1);
ub = [ones(n,1)*max(model.ub);ones(n,1)];
